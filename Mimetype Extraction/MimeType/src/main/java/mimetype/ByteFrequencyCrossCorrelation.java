...
public class ByteFrequencyCrossCorrelation {
	...
	public static void main(String[] args) {
		...
		for (File mimeTypeFolder : datasetPath.listFiles()) {
			...
			File folder = (mimeType == "application/octet-stream")?new File(folderPath + "/" + Constants.SEVENTY_FIVE_FOLDER): new File(folderPath + "/" + Constants.TWENTY_FIVE_FOLDER);
			...
			try {
				for (File file : folder.listFiles()) {
					if(mimeType=="application-octet-stream" && fileNumber >= ((folder.list().length)/3)){
						break;
					}
